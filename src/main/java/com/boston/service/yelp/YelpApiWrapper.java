...
public class YelpApiWrapper 
{
	...
	public String getCity(String busId) throws IOException, YelpException {
	...
	public List<Review> getReviews(String busId, String city) throws IOException, YelpException {
		...
		try {
			doGet(reviewUrl, config.getApiKey(), handlerWrapper((is) -> readReviews(is, city, reviews)));
		} 
		catch (MigratedBusinessException mbe) {
			busId = mbe.getNewBusId();
			reviewUrl = reviewUrl.replace("XX", busId);
	...
	// package level to allow for testing
